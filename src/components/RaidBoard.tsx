...
function SlotModal({ state, onClose, onRefresh, onQuickMove, onOptimisticMove, onOptimisticSkillUpdate, onOptimisticNameUpdate, onOptimisticJobUpdate, kind = 'main' }: {
  ...
  const unassigned = allAttendees
    .filter((a) => kind === 'practice' || a.sectionId === null)
    .filter((a) => a.memberName.toLowerCase().includes(query.toLowerCase()) || a.uid.includes(query))
    .filter((a) => !jobFilter || a.job === jobFilter)
  const isAlreadyPlaced = (a: Attendee) => kind === 'practice' ? a.practiceSectionId !== null : a.sectionId !== null
  ...
  async function assignExisting(uid: string) {
    ...
    fetch(`/api/attendees/${uid}/section`, {
      method: 'PUT', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ sectionId: state.sectionId, position: pos, kind }),
    })
  }
  ...
  async function removeFromSection() {
    ...
    fetch(`/api/attendees/${state.member.uid}/section`, {
      method: 'PUT', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ sectionId: null, position: null, kind }),
    })
  }
  ...
  async function swapMember(newUid: string) {
    ...
    Promise.all([
      ...
      fetch(`/api/attendees/${newUid}/section`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sectionId: state.sectionId, position: null, kind }),
      }),
    ])
  }
                    ...
                    <button key={a.uid} onClick={() => swapMember(a.uid)}
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          {isAlreadyPlaced(a) && <span className="text-xs text-yellow-400/80">• อยู่ในทีมอื่นแล้ว</span>}
                        </div>
                      </div>
...
export default function RaidBoard({ zoneNames, showSpecial, navLinks, kind = 'main' }: {
  zoneNames: string[]
  showSpecial: boolean
  ...
  async function handleDrop(targetSectionId: number, position?: number) {
    ...
    fetch(`/api/attendees/${uid}/section`, {
      method: 'PUT', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ sectionId: targetSectionId, position: position ?? null, kind }),
    })
  }
  ...
  async function handleClearSections(target: { label: string; sections: Section[] }) {
    ...
    for (const s of target.sections) fetch(`/api/sections/${s.id}`, {
      method: 'DELETE', headers: { 'Content-Type': 'application/json' },
  ...
  async function handleSwap(targetUid: string, targetSectionId: number, targetPosition: number | null) {
    ...
    Promise.all([
      fetch(`/api/attendees/${uid}/section`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sectionId: targetSectionId, position: targetPosition, kind }),
      }),
      fetch(`/api/attendees/${targetUid}/section`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sectionId, position, kind }),
      }),
    ])
  }
  ...
  // นับเฉพาะโซนของบอร์ดนี้ ไม่ปนกับบอร์ดอื่น
  const scopedSections = zones.filter((z) => zoneNames.includes(z.name)).flatMap((z) => z.sections)
  const laCount = scopedSections.filter((s) => s.name === 'ลา').reduce((n, s) => n + s.attendees.length, 0)
  const sarongCount = scopedSections.filter((s) => s.name === 'สำรอง').reduce((n, s) => n + s.attendees.length, 0)
  const totalCount = scopedSections.reduce((n, s) => n + s.attendees.length, 0)
  const warAttendees = scopedSections.filter((s) => !SPECIAL.includes(s.name)).flatMap((s) => s.attendees)
  const jobCounts = JOBS.map((job) => ({ job, count: warAttendees.filter((a) => a.job === job).length }))

  const TABS: ViewMode[] = ['ทั้งหมด', ...zoneNames, ...(showSpecial ? ['ลา', 'สำรอง'] : [])]
  ...
  const teamZones = view === 'ทั้งหมด'
    ? zoneNames.map((n) => zoneOf(n)).filter(Boolean) as Zone[]
    : (view === 'ลา' || view === 'สำรอง') ? [] : [zoneOf(view)].filter(Boolean) as Zone[]

  const specialSections = (view === 'ลา' || view === 'สำรอง')
    ? zones.filter((z) => zoneNames.includes(z.name)).flatMap((z) => z.sections.filter((s) => s.name === view))
    : []
    ...
    <main ref={captureRef} className={`${view === 'ทั้งหมด' ? 'min-h-screen overflow-y-auto' : 'h-screen overflow-hidden'} px-3 py-3 relative`}
            ...
            {showSpecial && (
              <div className="flex flex-col sm:flex-row gap-2">
                {(['สำรอง', 'ลา'] as const).map((label) => {
                  const secs = zones.filter((z) => zoneNames.includes(z.name)).flatMap((z) => z.sections.filter((s) => s.name === label))
                  return (
      ...
      {capturing && (
        <div className="fixed top-3 right-3 z-50 px-4 py-2 rounded-full text-sm font-semibold text-white bg-black/80 border border-green-400/50 backdrop-blur shadow-lg">
          📷 กำลังถ่าย {capturing.replace('Team', '')}... {captureProgress}%
        </div>
      )}

      {/* Slot Modal */}
      {slotModal && (
        <SlotModal state={slotModal} onClose={() => setSlotModal(null)} onRefresh={load} onQuickMove={showSpecial ? handleQuickMove : undefined} onOptimisticMove={optimisticMoveAttendee} onOptimisticSkillUpdate={optimisticSkillUpdate} onOptimisticNameUpdate={optimisticNameUpdate} onOptimisticJobUpdate={optimisticJobUpdate} kind={kind} />
      )}
